function [ Sxx, Sxy, Syx, Syy ] = AssembleStiffMatrix( coef, JField, Nodes, P, T, r, s, w )
    Enodes = size( T, 2 );
    Sxx= sparse( Nodes, Nodes );
    Sxy= sparse( Nodes, Nodes );
    Syx= sparse( Nodes, Nodes );
    Syy= sparse( Nodes, Nodes );
    Elements = size( T, 1 );
    ...
    for in = 1 : Enodes
        phiR( :, in ) = derivPhi( coef, r, s, in, 'R' );
        phiS( :, in ) = derivPhi( coef, r, s, in, 'S' );
    end
    ...
    for i = 1 : Elements
        ...
        Sxx( T(i, :), T(i, :) ) = Sxx( T(i, :), T(i, :) ) + abs( J ) * Sexx;
        Sxy( T(i, :), T(i, :) ) = Sxy( T(i, :), T(i, :) ) + abs( J ) * Sexy;
        Syx( T(i, :), T(i, :) ) = Syx( T(i, :), T(i, :) ) + abs( J ) * Seyx;
        Syy( T(i, :), T(i, :) ) = Syy( T(i, :), T(i, :) ) + abs( J ) * Seyy;
